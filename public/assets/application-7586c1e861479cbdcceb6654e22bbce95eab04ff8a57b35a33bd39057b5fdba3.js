...
$(function () {
  ...
  $('#links-filter').on('keyup', function (event) {
    Link.filterLink(event);
  });

  $('#filter-unread-btn').on('click', function (event) {
    Link.filterUnread(event);
  });

  $('#filter-read-btn').on('click', function (event) {
...
function updateHotReads() {
  $.ajax({
    ...
    var urls = reads.map(function (read) {
      return read.link_url;
    });

    $('.card').each(function (i, card) {
      var url = $(card).find('a')[0].text.toLowerCase();
...
function updateLinkStatus(link) {
  var card = $('.card[data-id=' + link.id + ']');
  var status = card.find('.read-status');
  var buttonText = card.find('.mark-as-read');
...
function createRead(link) {
  ...
  }).then(updateHotReads).fail(displayFailure);
}
